...
{    
    ...
    public function home(Request $request){
      ...
      $locale='en';
        $data_banner =Banner::orderBy('id','DESC')->select('banner_image', 'banner_title', 'banner_desc', 'redirect_url', 'banner_mobile_image')->limit(1)->where('lang_code', $locale)->get();  
        $testimonials = Testimonial::orderBy('id','DESC')->limit(4)->where('lang_code', $locale)->get();
    ...
    public function infographics(Request $request){
         $infographic = Infographic::orderBy('id', 'DESC')->paginate(12); // or ->simplePaginate(10) for simpler controls

       if ($request->ajax()) {
        return view('infographics-list', compact('infographic'))->render();
    }
        /* wrong pagination redirect to 404 page */
        $no_of_page = 0;
    ...
    public function press_release(Request $request){
          ...
          $press = Press::orderBy('id', 'DESC')->paginate(10); // or ->simplePaginate(10) for simpler controls

    if ($request->ajax()) {
        return view('press_list', compact('press'))->render();
    }
   $no_of_page = 0;
    ...
    public function research_report_main(Request $request){
        
            $data = Report::paginate(10); // or ->simplePaginate(10) for simpler controls

    if ($request->ajax()) {
        return view('research_report_main_list', compact('data'))->render();
    }
       
        $category = Category::where('lang_code','en')->orderBy('cat_name','ASC')->get();
    ...
    public function research_report_details(Request $request,$page_url){
       // echo "hello"; die;
// $testimonials = Testimonial::orderBy('id','DESC')->limit(2)->get();
 $testimonials = Cache::remember('daily_shuffled_testimonials_' . now()->format('Y-m-d'), now()->addDay(), function () {
            return Testimonial::inRandomOrder()->limit(2)->get();
        });
      
      $report_slug_check = Report::where('page_url',$page_url)->count();
      $report_slug_category_check = Category::where('category_url',$page_url)->where('lang_code','en')->count();
...
$sub_category_h2 = $sub_category->h2??'';
      if($report_slug_check > 0) {
       ...
       // echo $datas->upcomingstatus; die;
        ...
        if($datas->upcomingstatus == '1' ){         
          return view('research_report_details',compact('datas','related_reports', 'sub_category_h2','testimonials'));
        }
        else{ 
               
          return view('research_report_details',compact('datas','related_reports','infographic_image', 'sub_category_h2','testimonials'));
        }
        
      }
      else if($report_slug_category_check > 0){          

          $category = Category::select('*')->where('category_url',$page_url)->where('lang_code','en')->first();
          $sub_category = Sub_Category::select('*')->where('cat_id',$category->id)->get();
    
          $sidebar_category = Category::where('lang_code','en')->orderBy('cat_name','ASC')->get();
     ...
     if ($request->ajax()) {
return view('categoryresearch_report_list',compact('category','sub_category','data','sidebar_category','sidebar_sub_category','region','testimonials'));
           }
          return view('categoryresearch_report',compact('category','sub_category','data','sidebar_category','sidebar_sub_category','region','testimonials'));
...
$sub_category_h2 = $sub_category[0]->h2;
        $category = Category::where('lang_code','en')->orderBy('cat_name', 'ASC')->where('id', $category_id)->first();
        $sidebar_category = Category::where('lang_code','en')->orderBy('cat_name', 'ASC')->get();
        $sidebar_sub_category = Sub_Category::orderBy('sub_cat_name', 'ASC')->get();
...
$region = Region::orderBy('region_name', 'ASC')->get();
 if ($request->ajax()) {
   return view('subresearch_report_list', compact('testimonials','category', 'sub_category', 'data', 'sidebar_category', 'sidebar_sub_category', 'region', 'subcat_id','subcat_name', 'sub_category_h2'));
 }
return view('subresearch_report', compact('testimonials','category', 'sub_category', 'data', 'sidebar_category', 'sidebar_sub_category', 'region', 'subcat_id','subcat_name', 'sub_category_h2'));
  ...
  if ($request->ajax()) {
        return view('regionreport_list', compact('testimonials','rigion1','sub_category','data','sidebar_category','sidebar_sub_category','region'))->render();
    }
    return view('regionreport',compact('testimonials','rigion1','sub_category','data','sidebar_category','sidebar_sub_category','region'));
...
$region = Region::orderBy('region_name', 'ASC')->get();
    $data = Report::where('country_id', $subcat_id)->orderBy('id', 'DESC')->paginate(10);
     if ($request->ajax()) {
        return view('country_report_list', compact('testimonials','category', 'sub_category', 'data', 'sidebar_category', 'sidebar_sub_category', 'region', 'subcat_id','subcat_name', 'subcat_name_h2'))->render();
    }
return view('country_report', compact('testimonials','category', 'sub_category', 'data', 'sidebar_category', 'sidebar_sub_category', 'region', 'subcat_id','subcat_name', 'subcat_name_h2'));
      ...
      elseif(strpos($page_url,'.xml')){
        ...
        $newCategoryArr = explode('.',$page_url);
            ...
            $category = Category::where('category_url', $newCategoryArr[0])->where('lang_code','en')->first();
            $subcategory = Sub_Category::where('page_url', $newCategoryArr[0])->first();
            $country = Country::where('page_url', $newCategoryArr[0])->first();
            $regions = Region::where('page_url', $newCategoryArr[0])->first();
            
            if ($category) {
            $data = Report::where('cat_id', $category->id)
                          ->where('upcomingstatus', '0')
                          ->orderBy('id', 'DESC')
                          ->get();
            } elseif ($subcategory) {
            $data = Report::where('sub_cat_id', $subcategory->id) // assuming country_id is the correct column
                          ->where('upcomingstatus', '0')
                          ->orderBy('id', 'DESC')
                          ->get(); 
            } elseif ($country) {
            $data = Report::where('country_id', $country->id) // assuming country_id is the correct column
                          ->where('upcomingstatus', '0')
                          ->orderBy('id', 'DESC')
                          ->get();
            } elseif ($regions) {
            $data = Report::where('region_id', $regions->id)
                          ->where('upcomingstatus', '0')
                          ->orderBy('id', 'DESC')
                          ->get();
            } else {
            $data = collect(); // empty collection if nothing matches
            }
   ...
   public function save_contact(Request $req)
    ...
    public function services(){
        ...
        $industry = Category::where('lang_code','en')->orderBy('id','desc')->get();
    ...
    public function research_library_categorywise($category_url){
      $category = Category::select('*')->where('category_url',$category_url)->where('lang_code','en')->first();
      $sub_category = Sub_Category::select('*')->where('cat_id',$category->id)->get();
