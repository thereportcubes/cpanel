...
<script type="text/javascript">
  ...
  <?php 
   $seo_data = "";
   $seo_slug_url = "";
    $fullUrl = url()->full();           // Includes query params
    $currentUrl = url()->current();     // Excludes query params
    $page = request()->get('page');     // Get page number from query string
...
if(!is_null($parent_id)){
   ?>
      ...
      <base href="<?php echo $canonicalUrl; ?>">
   ...
   <?php }}
      ...
      $seo_data =\DB::table('seo_content')->where(['page_type'=>'infographics','static_pages'=>'1'])->orderBy('id','DESC')->first(); 
   ?>         
   ...
   else if($seo_slug_url_type == "shopping-basket" && $seo_slug_url == ""){ 
      $seo_data =\DB::table('seo_content')->where('page_type','shopping-basket')->orderBy('id','DESC')->first(); 
   ?>
   ...
   else if($seo_slug_url_type == "signin" && $seo_slug_url == ""){ 
      $seo_data =\DB::table('seo_content')->where('page_type','signin')->orderBy('id','DESC')->first(); 
   ?>
   ...
   else if($seo_slug_url_type == "forget-password" && $seo_slug_url == ""){ 
      $seo_data =\DB::table('seo_content')->where('page_type','forget-password')->orderBy('id','DESC')->first(); 
   ?>
   ...
   <?php } else if($seo_slug_url_type == app()->getLocale() && $seo_slug_url == ""){ 
      $seo_data =\DB::table('seo_content')->where('page_type','home')->orderBy('id','DESC')->first(); 
   ?>
        ...
        if($report_slug_check > 0) {
            ...
            ?>
                <script type="application/ld+json">
            	{
            		...
            		"url": "https://www.thereportcubes.com/report-store/{{@$parent_id->page_url}}",
            		"sameAs": "https://www.thereportcubes.com/report-store/{{@$parent_id->page_url}}",
            		"license": "https://www.thereportcubes.com/privacy-policy",
...
<?php } else if($seo_slug_url_type == "request-sample" && $seo_slug_url !== ""){ 
      $seo_page_url_request = request()->segment(2);
       $request_slug_check = \DB::table('reports')->where('page_url',$seo_page_url_request)->count();
         if($request_slug_check > 0) {
      $parent_id =\DB::table('reports')->where('page_url',$seo_page_url_request)->first();      
      $seo_data =\DB::table('seo_content')->where('parent_id',$parent_id->id)->where('page_type','report')->orderBy('id','DESC')->first(); 
   
   ?>         
   
      <title>Request Sample -<?php echo $seo_data->seo_title ;?></title>
      <meta name="description" content="Request Sample -<?php echo $seo_data->seo_description ;?>" />
      <meta name="keywords" content="Request Sample -<?php echo $seo_data->seo_key_words ;?>">
      <link rel="canonical" href="<?php echo $canonicalUrl; ?>" />
...
<body >
   <?php
   $allcategory = \DB::table('category')->orderBy('cat_name', 'ASC')->where('lang_code', app()->getLocale())->get();
        ...
        <ul class="text-right right">
            ...
            @if (Auth::check())
               <li class="hide-phonemin" style="cursor:pointer">
                  <a href="#"  >{{ __('common.welcome', ['name' => auth()->user()->first_name . ' ' . auth()->user()->last_name]) }}</a><!-- Display user's name -->
                </li>
            
            @else
                <li class="hide-phonemin" style="cursor:pointer">
                    <a href="{{ route('signin', ['locale' => app()->getLocale()]) }}">{{ __('common.sign_in') }}/{{ __('common.register') }}</a> <!-- Sign-in/Register link -->
                </li>
            @endif
         ...
         <div class="container">
            <div class="row align-items-center ">
               <div class="col-md-2  hide-on-mobile">
                  <a class="navbar-brand d-inline-block hide-on-mobile" href="{{ route('home', ['locale' => app()->getLocale()]) }}" rel="noopener noreferrer"  aria-label="logo">
               ...
               <div class="col-md-10">
                  <nav class="navbar navbar-expand-lg navbar-light white-bg pt-2 pb-1 "
                        >
                            <div class="container">
                                <a class="navbar-brand hide-on-desktop" href="https://www.thereportcubes.com/"
                                    ...
                                    <ul class="navbar-nav">
                                        <li class="nav-item dropdown">
                                            <a class="nav-link dropdown-toggle  " rel="noopener noreferrer"
                                                ...
                                                {{ __('common.what_we_do') }}
                                            </a>
                                            <ul class="dropdown-content" aria-labelledby="navbarDropdown" style="list-style: none;">
                                                <li><a class="dropdown-item"
                                                        href="{{url('syndicated-research')}}"
                                                        rel="noopener noreferrer" aria-label="Cart Name">{{ __('common.syndicated_research') }}</a>
                                                </li>
                                                <li><a class="dropdown-item"
                                                     href="{{url('customized-research')}}"
                                                        rel="noopener noreferrer" aria-label="Cart Name">{{ __('common.customized_research') }}</a></li>
                                                <li><a class="dropdown-item"
                                                       href="{{url('competitive-analysis')}}"
                                                        rel="noopener noreferrer" aria-label="Cart Name">{{ __('common.competitive_analysis') }}</a></li>
                                                <li><a class="dropdown-item"
                                                        href="{{url('company-profile')}}"
                                                        rel="noopener noreferrer" aria-label="Cart Name">{{ __('common.company_profile') }}</a></li>
                                             <li><a class="dropdown-item"
                                                        href="{{url('biographies')}}"
                                                        rel="noopener noreferrer" aria-label="Cart Name">{{ __('common.biographies') }}</a></li>
                                        ...
                                        <li class="nav-item">
                                            <a class="nav-link " aria-current="page"
                                                ...
                                                aria-label="Upcoming">{{ __('common.about_us') }}</a>
                                        </li>
                                       <li class="nav-item dropdown">
                                    ...
                                    <!--     <li class="nav-item">
                                        ...
                                        <li class="nav-item">
                                            <a class="nav-link " href="{{url('press-release')}}"
                                                rel="noopener noreferrer" aria-label="Infographics">{{ __('common.press_release') }}</a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link " href="{{url('contact-us')}}">{{ __('common.contact_us') }}</a>
                                        </li>
                ...
                <div class="modal-body">
                  <form action="{{url('search/search-result')}}" method="get" class="search" id="mainsearch">
                     ...
                     <div class="col-md-12 mb-3">
                        <p class="mb-0">{{ __('common.search') }}</p>
                     ...
                     <div class="col-md-12 mt-2">
                        <button type="submit" class="ab-button ab-button-primary ab-button-small">{{ __('common.search') }}</button>
                     </div>
